...
if [[ -n "$source_env" ]]; then
    ...
    if ! grep -q '^PRIVACY_NOTICE_VERSION=' "$application_tmp"; then
        printf '%s\n' 'PRIVACY_NOTICE_VERSION=2026-09-11.1' >>"$application_tmp"
    fi
    if ! grep -q '^AGENT_MAIL_ENABLED=' "$application_tmp"; then
        printf '%s\n' 'AGENT_MAIL_ENABLED=false' >>"$application_tmp"
    fi
    if ! grep -q '^OUTREACH_ENABLED=' "$application_tmp"; then
        printf '%s\n' 'OUTREACH_ENABLED=false' >>"$application_tmp"
    fi
    if ! grep -q '^OUTREACH_MAX_RECIPIENTS=' "$application_tmp"; then
        printf '%s\n' 'OUTREACH_MAX_RECIPIENTS=5' >>"$application_tmp"
    fi
    if ! grep -q '^OUTREACH_CANDIDATE_RETENTION_DAYS=' "$application_tmp"; then
        printf '%s\n' 'OUTREACH_CANDIDATE_RETENTION_DAYS=30' >>"$application_tmp"
    fi
    ...
    printf '%s\n' \
        ...
        'PRIVACY_NOTICE_VERSION=2026-09-11.1' \
        >>"$application_tmp"
fi
