...
def _resend_artifact_id(tags) -> str | None:
    if isinstance(tags, dict):
        value = tags.get("laggente_email_id")
        return value if isinstance(value, str) else None
    if isinstance(tags, list):
        for tag in tags:
            if (
                isinstance(tag, dict)
                and tag.get("name") == "laggente_email_id"
...
def _record_resend_delivery_event(
    db: Session,
    *,
    event_type: str,
    data: ResendEmailDeliveryData,
) -> None:
    artifact_id = _resend_artifact_id(data.tags)
    identifiers = [
        and_(
            ProfessionalEmail.provider == "resend",
            ProfessionalEmail.provider_message_id == data.email_id,
        ),
    ]
    if artifact_id:
        identifiers.append(
            and_(
                ProfessionalEmail.id == artifact_id,
                ProfessionalEmail.provider.in_([None, "resend"]),
            )
        )
    email = db.scalar(
    ...
    if not email:
        # Authentication and invitation emails use the same Resend webhook but are not sealed
        # professional artifacts. Resend's suppression list remains authoritative for them.
        return
    next_status = RESEND_DELIVERY_STATUSES[event_type]
    if RESEND_STATUS_PRIORITY.get(next_status, 0) <= RESEND_STATUS_PRIORITY.get(
        email.status, 0
    ):
        return
    email.provider = "resend"
    email.provider_message_id = data.email_id
    email.status = next_status
    email.failure_code = (
        f"resend_{next_status}"
    ...
    db.add(
        Event(
            ...
            actor_type="system",
            event_type=f"professional_email_{next_status}",
            payload={
                "email_id": email.id,
                "provider": "resend",
    ...
    notices = {
        "delivery_delayed": f"La consegna dell'email a {email.to_address} è in ritardo.",
        "failed": f"L'email a {email.to_address} non è stata consegnata.",
        "bounced": f"L'indirizzo {email.to_address} ha rifiutato definitivamente l'email.",
        "suppressed": f"Resend ha bloccato l'invio a {email.to_address} per proteggere la reputazione.",
        "complained": f"Il destinatario {email.to_address} ha segnalato l'email come indesiderata.",
    }
    if notice := notices.get(next_status):
        db.add(
            Message(
                account_id=email.account_id,
                conversation_id=email.studio_conversation_id,
    ...
    if not provider_state_already_recorded:
        ...
        db.add(
            Event(
                ...
                actor_type="system",
                event_type=(
                    "professional_email_sent"
                    if result.delivered
                    else "professional_email_simulated"
                ),
                payload={
                    "email_id": email.id,
...
async def ingest_resend_professional_email(
    ...
    if event.type in RESEND_DELIVERY_STATUSES:
        try:
            delivery_data = ResendEmailDeliveryData.model_validate(event.data)
        except ValidationError as exc:
            raise HTTPException(status_code=422, detail="Evento di consegna non valido") from exc
        _record_resend_delivery_event(
            db,
